...
def seq2seq(x,y,keep_prob,predict,dim_lstm,\
            encoder_vocab=None, encoder_word_embd_pretrain=None,encoder_vocab_size=None,word_embd_dim=None,\
            decoder_vocab=None, decoder_word_embd_pretrain=None,decoder_vocab_size=None):
...
def get_word_embd(word_embd_pretrain=[], vocab_size=None, word_embd_dim=None, name='word_embd'):
    # word_embd:[batch_size,step_len,embd_dim]
    if [] != word_embd_pretrain:
        word_embd = tf.get_variable(name=name, trainable=True, initializer=word_embd_pretrain)
        vocab_size = word_embd.shape[0].value
        word_embd_dim = word_embd.shape[1].value
    elif None==vocab_size or None==word_embd_dim:
        ...
        word_embd = tf.get_variable(name=name, trainable=True,
                                    initializer=tf.random_uniform((vocab_size, word_embd_dim), -1, 1,
                                                                          dtype=tf.float32))
