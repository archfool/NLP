...
# In the future, it would make more sense to write variants on the attention mechanism using the new seq2seq_nmt library for tensorflow 1.0: https://www.tensorflow.org/api_guides/python/contrib.seq2seq#Attention
def attention_decoder(decoder_inputs, initial_state, encoder_states, enc_padding_mask, cell, initial_state_attention=False, pointer_gen=True, use_coverage=False, prev_coverage=None):
  """
  Args:
    decoder_inputs: embd后的目标序列，按时序unstack为list。A list(length [max_dec_steps]) of 2D Tensors [batch_size, emb_dim].
    initial_state: encoder传入decoder的初始状态。2D Tensor [batch_size , cell.state_size].
    encoder_states: encoder的outputs，即memory。3D Tensor [batch_size , attn_length , 2*hidden_dim].
    enc_padding_mask: encoder的time_step的mask。2D Tensor [batch_size , attn_length] containing 1s and 0s; indicates which of the encoder locations are padding (0) or a real token (1).
    cell: decoder的RNN结构。rnn_cell.RNNCell defining the cell function and size.
    initial_state_attention:
      Note that this attention_decoder passes each decoder_input through a linear layer with the previous step's context
      vector(state) to get a modified version of the input.
      If initial_state_attention is False, on the first decoder step the "previous context vector(state)" is just a zero vector.
      If initial_state_attention is True, we use initial_state to (re)calculate(replace) the previous step's context vector(state).
  ...
  Returns:
    outputs: A list of the same length as decoder_inputs of 2D Tensors of
      shape [batch_size, cell.output_size(dim)]. The output vectors.
    state: The final state of the decoder. A tensor shape [batch_size, cell.state_size].
    attn_dists: 类似PtrNet的词典。A list containing tensors of shape (batch_size, attn_length).
  ...
  with variable_scope.variable_scope("attention_decoder") as scope:
    ...
    # dim_attention(attn_size)=2*hidden_dim
    ...
    def attention(decoder_state, coverage=None):
      ...
      Args:
        decoder_state: state of the decoder.2D Tensor [batch_size, cell.state_size].
    ...
    if initial_state_attention: # true in decode mode
      # Re-calculate the context vector from the previous step so that we can pass it through a linear layer with this
      # step's input to get a modified version of the input
      # in decode mode, this is what updates the coverage vector
      # todo perhaps this step performs as <sos> step ???
...
def linear(args, output_size, bias, bias_start=0.0, scope=None):
  ...
  Args:
    args: a 2D Tensor or a list of 2D, [batch, n], Tensors. When data is multi-layers, it should be a list.
