...
from nn_lib.ptrnet_attention_decoder import attention_decoder
...
class SummarizationModel(object):
  ...
  def _add_decoder(self, inputs):
    ...
    outputs, out_state, attn_dists, p_gens, coverage = \
      attention_decoder(inputs, self._dec_in_state, self._enc_states, self._enc_padding_mask, cell,
                        initial_state_attention=(hps.mode=="decode"), pointer_gen=hps.pointer_gen,
  ...
  def _calc_final_dist(self, vocab_dists, attn_dists):
    ...
    with tf.variable_scope('final_distribution'):
      ...
      # This means that if a_i = 0.1 and the ith encoder word is w, and w has index 500 in the vocabulary, then we add
      # 0.1 onto the 500th entry of the final distribution
      # This is done for each decoder timestep.
      # This is fiddly; we use tf.scatter_nd to do the projection
      batch_nums = tf.range(0, limit=self._hps.batch_size) # shape (batch_size)
      batch_nums = tf.expand_dims(batch_nums, 1) # shape (batch_size, 1)
      attn_len = tf.shape(self._enc_batch_extend_vocab)[1] # number of states we attend over(step_len?)
  ...
  def _add_seq2seq(self):
    ...
    with tf.variable_scope('seq2seq_nmt'):
      # Some initializers
