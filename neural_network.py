...
class neural_network(object):
    ...
    def __init__(self,data_x,data_y,task_type=None,\
                 model_type=None,loss_fun_type=None,eval_score_type=None,
                 optimizer_type='Adam',hyper_parameter={},model_parameter={},\
                 path_data=''):
        #规整样本特征值x和预测值y
        self.init_data(data_x, data_y)
        #配置神经网络的类型：回归，二分类、多分类等等
        if self.init_task_type(task_type)==None:
            return
        #检测hyper_parameter和model_parameter参数是否为字典
        ...
        #配置目标函数类型
        self.init_loss_fun_type(loss_fun_type)
        #配置评价函数的类型
        self.init_eval_score(eval_score_type,hyper_parameter=hyper_parameter)
        #配置超参数
        #初始化优化器
        ...
        self.model_save_epoch = hyper_parameter.get('model_save_epoch', 100)
    ...
    '''========================================================================'''
    #训练
    def train(self,x=None,y=None,loop=100000,transfer_learning=False,\
...
#                        self.tmp2a,self.tmp2b = sess.run([self.model_out,self.y_ph],feed_dict=feed_dict_test)
...
#        model_path = tf.train.latest_checkpoint(os.path.join(self.path_data,'model_save'))
    ...
    #配置评估函数的类型和评估监控器
    def init_eval_score(self, eval_score_type, hyper_parameter={}):
        if not eval_score_type:
            if self.task_type=='regression':
        ...
        self.built_in_test_epoch = hyper_parameter.get('built_in_test_epoch', 20)
    ...
    def early_stop_judge(self, new_score):
        #判断是否使用early_stop
        if self.early_stop_rounds==None:
            self.early_stop_flag = False
            return
        #判断评估函数值是大点好，还是小点好
...
def demo_regression():
    ...
    model = neural_network(x,y,task_type='regression',\
...
#    model=neural_network(x,y,dim=[5,5,5,2],batch_size=1024,keep_prob=1.0,\
